...
begin
  ...
  for i:=1 to k do
    begin
      ...
      if ch='<' then
        begin
          for j:=1 to p do if v[b[j]]=-maxlongint then v[b[j]]:=-i
                              else if v[b[j]]<0 then v[b[j]]:=-i
                                       else if v[b[j]]>0 then v[b[j]]:=0;
          for j:=1 to p do if v[c[j]]=-maxlongint then v[c[j]]:=i
                              else if v[c[j]]>0 then v[c[j]]:=i
                                       else if v[c[j]]<0 then v[c[j]]:=0;
