...
bool cmp(point a,point b){
	...
	a.y-=p.y;
	b.y-=p.y;
	if (abs(mul(a,b))) return (mul(a,b)>0);
...
int main(){
	...
	for (int i=1;i<r;i++) ans+=sqrt(sqr(a[f[i]].x-a[f[i+1]].x)+sqr(a[f[i]].y-a[f[i+1]].y)+0.0);
	ans+=sqrt(sqr(a[f[1]].x-a[f[r]].x)+sqr(a[f[1]].y-a[f[r]].y)+0.0);
	ans+=3.14159265*2*l;
	printf("%.0f",ans);
}